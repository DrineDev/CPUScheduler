...
public class LongestJobFirst extends Algorithm {
    public LongestJobFirst(int numCores) {
        super(numCores);
        // Priority queue sorted by burst time in descending order (longest first)
        readyQueue = new PriorityQueue<>(Comparator.comparingInt(Task::getBurstTime).reversed());
    }
